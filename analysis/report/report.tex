...
\renewcommand{\stamptime}{2026-08-15 13:01 PDT}
\renewcommand{\stampversion}{\detokenize{e768e47-wip}}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{booktabs}
...
alternatives. We consider three
canonical settings:
...
\item
  \textbf{Constrained longitudinal data analysis versus
...
parameterization \texttt{y\ \textasciitilde{}\ 0\ +\ tg} (where \texttt{tg} is a
...
dependence on \texttt{emmeans} or other contrast packages.
...
inline R from the \texttt{summary\_prepost} and \texttt{summary\_3tp} data
...
\caption{Simulation-by-simulation comparison of treatment effect estimates from ANCOVA and MMRM (\(\rho = 0.6\), \(\sigma_2/\sigma_1 = 1\)). Points on the identity line indicate exact agreement.}
\end{figure}
...
\caption{Standard errors from ANCOVA versus MMRM across all nine scenarios. Identity line shown as dashed.}
\end{figure}
...
diverge. This confirms the\textsuperscript{\citeproc{ref-frost2008optimizing}{5}}
...
\caption{Ratio of standard errors (summary statistic / random slopes) as a function of the between-subject slope SD to residual SD ratio. Values near 1 indicate the two methods are equivalent.}
\end{figure}
...
\textbf{Condition 2: small between-subject slope variance.}
...
small. As\textsuperscript{\citeproc{ref-frost2008optimizing}{5}} showed, when this ratio
...
\textbf{Condition 3: cLDA with complete data.} The
constrained longitudinal model produces treatment
effect estimates effectively identical to ANCOVA when
no baseline values are missing\textsuperscript{\citeproc{ref-liu2009baseline}{9},\citeproc{ref-lu2010efficiency}{11}}. The marginal advantage of cLDA
...
proposed by Morris, White, and Crowther\textsuperscript{\citeproc{ref-morris2019using}{23}}. The full
...
\tightlist
\item
  Simulation code lives entirely inside the Rmd; no \texttt{R/} package functions or scripts.
\item
  \texttt{n\_sims\ =\ 2000} hardcoded at two places without MCSE-based justification.
\item
  Seeds are set via function arguments (\texttt{seed\ =\ 42}, \texttt{seed\ =\ 123}), not once at program start; no MCSE columns in \texttt{summary\_prepost()} or \texttt{summary\_3tp()}.
\end{itemize}
